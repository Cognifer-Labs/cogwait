'use strict';
// Sponsoric ad levels — the single source of truth shared by the client
...
//
// The developer picks the level (SPONSORIC_LEVEL / config `level` / `--level N`).
...
const LEVELS = [
  ...
  { id: 1, key: 'minimal',  label: 'Minimal',  lines: 1, cpm: envNum('SPONSORIC_CPM_L1', 8),
    desc: 'One dim, single-line sponsor note in the status row. The default — barely there.' },
  { id: 2, key: 'standard', label: 'Standard', lines: 1, cpm: envNum('SPONSORIC_CPM_L2', 18),
    desc: 'A brighter, colored single line with an icon and a clear call-to-action.' },
  { id: 3, key: 'boosted',  label: 'Boosted',  lines: 2, cpm: envNum('SPONSORIC_CPM_L3', 35),
    desc: 'A two-line boxed sponsor block with headline + link. The most prominent, best-paid tier.' }
];
