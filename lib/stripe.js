...
// opts.idempotency_key, when present, is forwarded as Stripe's Idempotency-Key
// request header on the real call so a retried request never double-transfers
// server-side. On the simulated (no-key) path it's a no-op — the caller's own
// pending/settled marker is what carries idempotency there (see server/index.js).
function transfer(pid, amountUsd, opts, cb) {
  ...
  const idempotencyKey = opts && opts.idempotency_key;
...
function onboard(pid, cb) {
  ...
  api('POST', 'https://api.stripe.com/v1/accounts', { type: 'express' }, (e1, acct) => {
    ...
    const params = {
      ...
      refresh_url: (process.env.COGWAIT_CONNECT_REFRESH || 'https://cogwait.io/connect/refresh'),
      return_url: (process.env.COGWAIT_CONNECT_RETURN || 'https://cogwait.io/connect/done')
    };
