'use strict';
// Cogwait API client — dependency-free, privacy-preserving.
...
const STATE_DIR = path.join(os.homedir(), '.cogwait');
const CONFIG_PATH = path.join(STATE_DIR, 'config.json');

// Config precedence: env var > ~/.cogwait/config.json > default.
function loadConfig() {
  let cfg = {};
...
const API_BASE = pick('COGWAIT_API', 'api', 'https://api.cogwait.io');
const PAYOUT_ID = pick('COGWAIT_PAYOUT_ID', 'payout_id', '') || '';
const PUBLISHER_KEY = pick('COGWAIT_PUBLISHER_KEY', 'publisher_key', '') || '';
const MOCK = String(pick('COGWAIT_MOCK', 'mock', '')) === '1';
const DISABLED = String(pick('COGWAIT_DISABLED', 'disabled', '')) === '1';
...
const LEVEL = levels.clampLevel(pick('COGWAIT_LEVEL', 'level', levels.DEFAULT_LEVEL));

// Fund-OSS give-back %. Display-only here — the server clamps and enforces
// the real value at /payout; this local copy (env/config) is for the CLI/UI to
// show without a network round trip. Default mirrors the server's on-by-default 20%.
function clampPct(n) {
  const v = Math.round(Number(n));
  return Number.isFinite(v) ? Math.max(0, Math.min(100, v)) : 20;
}
const DONATE_PCT = clampPct(pick('COGWAIT_DONATE_PCT', 'donate_pct', 20));
...
function request(method, urlStr, body, timeoutMs, cb) {
  ...
  const req = lib.request(u, {
    ...
    headers: Object.assign(
      { 'Accept': 'application/json', 'User-Agent': 'cogwait/0.1' },
...
// ---- state-dir garbage collection --------------------------------------
// Every session leaves behind an ad cache, an impression stamp, and a pair of
// wait timestamps. Nothing ever removed them, so ~/.cogwait grew one set per
// session forever. Prune only those per-session artifacts — config.json,
// oss-receipt.json, refresh.fail and anything else stay put, and the pattern is
// an allowlist so a future file type is ignored rather than deleted by accident.
const GC_STAMP = path.join(STATE_DIR, 'gc.stamp');
const GC_INTERVAL_MS = 60 * 60 * 1000;          // scan at most once an hour
const GC_MAX_AGE_MS = 48 * 60 * 60 * 1000;      // a session's cache is dead long before this
const GC_PATTERN = /^(?:ad-[0-9a-f]+\.json|imp-[0-9a-f]+\.stamp|wait-[0-9a-f]+\.(?:start|end))$/;

// Cheap, silent, best-effort. Called from refreshAd(), which only ever runs in
// the detached refresher child — the statusline process never does this work,
// and the stamp check short-circuits before any readdir. Never throws.
function gcState(now) {
  const t = now || Date.now();
  try {
    const st = fs.statSync(GC_STAMP);
    if (t - st.mtimeMs < GC_INTERVAL_MS) return 0;
  } catch (_) { /* no stamp yet — first run */ }
  // Stamp first: if the sweep dies halfway we still wait an hour to retry
  // instead of rescanning on every refresh.
  try { writeSecret(GC_STAMP, ''); } catch (_) { return 0; }
  ...
  for (const name of names) {
    ...
    try {
      if (t - fs.statSync(fp).mtimeMs < GC_MAX_AGE_MS) continue;
      fs.unlinkSync(fp);
      removed++;
    } catch (_) { /* raced with another process; fine */ }
  }
  return removed;
}
...
function spawnRefresh(sessionId) {
  ...
  try {
    ...
    const child = spawn(process.execPath, [path.join(__dirname, '..', 'bin', 'refresh-ad.js')], {
      env: Object.assign({}, process.env, { COGWAIT_SESSION: String(sessionId || '') }),
      detached: true,
      stdio: 'ignore'
    });
    child.unref();
...
function refreshAd(sessionId, done) {
  const tag = sessionTag(sessionId);
  try { gcState(); } catch (_) {}   // piggyback the sweep on the off-hot-path refresher
...
function reportImpression(sessionId, ad, shownMs) {
  ...
  // rather than send an unauthenticated request. Register with `npx cogwait --register`.
...
// Set the Fund-OSS give-back percentage server-side. Body carries ONLY the
// percentage — never any dependency name, URL, or file content (that scan
// output never leaves the machine; see lib/oss.js / bin/oss.js).
function setDonatePct(pct, cb) {
  request('POST', `${API_BASE}/donate/config`, { donate_pct: clampPct(pct) }, 5000, cb);
}

// Start Stripe Connect onboarding for the authed publisher — the "connect your
// bank account or card" rail. Returns the hosted onboarding URL to open.
function connectStripe(cb) {
  request('POST', `${API_BASE}/connect/onboard`, {}, 10000, cb);
}

// Choose the cashout rail: 'stripe' (bank/card via Connect) or 'paypal' (to an
// email). The server validates the method + email and clamps what it stores.
function setPayoutMethod(method, email, cb) {
  const body = email ? { method, paypal_email: email } : { method };
  request('POST', `${API_BASE}/payout/method`, body, 5000, cb);
}

// Request a payout of the whole balance. The server splits it into a cashout
// leg and a Fund-OSS donation leg under a row lock and is idempotent on retry —
// the client sends no amounts at all, so it cannot influence the split.
function requestPayout(cb) {
  request('POST', `${API_BASE}/payout`, {}, 30000, cb);
}

module.exports = {
  getCachedAd, refreshAd, reportImpression, sessionTag,
  inBackoff, recordFailure, clearFailures, request,
  secureDir, writeSecret, configIsTrustworthy, setDonatePct, clampPct,
  connectStripe, setPayoutMethod, requestPayout, gcState,
  STATE_DIR, CONFIG_PATH, API_BASE, PAYOUT_ID, PUBLISHER_KEY, MOCK, DISABLED, CHAIN, LEVEL, DONATE_PCT
};
