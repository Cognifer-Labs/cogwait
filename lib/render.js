...
// Unpaid AI-news fallback line — rendered only when there is no ad. Always the
// minimal single-row treatment regardless of level: nobody is paying for this
// slot, so it never earns extra prominence, and it is never labeled [sponsor].
function renderNews(item) {
  const { DIM, CYAN, RESET } = A;
  const text = `${DIM}${item.title}${RESET}`;
  return item.url
    ? `${CYAN}[news]${RESET} ${osc8(item.url, `${text} ›`)}`
    : `${CYAN}[news]${RESET} ${text}`;
}

module.exports = { renderAd, renderInline, renderPlain, renderNews };
