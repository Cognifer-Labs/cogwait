# PRD: Cogwait — Honest Wait-Time Sponsorship for Claude Code

**Owner:** Cognifer Labs LLC · **Target:** Q3 2026 · **Persona:** Claude Code CLI developers

## 1. Summary
Cogwait monetizes developer wait time by rendering a **visible, labeled sponsor line** in Claude Code while the agent works, then sharing ad revenue with the developer. Impressions count **only when actually shown to a human** — no phantom impressions, no hidden calls. Distributed as a Claude Code plugin.
...
- **Hooks (impression timing).** `UserPromptSubmit` marks wait start; `Stop` fires when Claude finishes responding; `Notification`/`idle_prompt` marks idle. An `http`-type hook POSTs event JSON directly to the Cogwait backend — no local relay needed.
...
## 4. Cogwait API
- `POST /session/init` — verify `COGWAIT_PAYOUT_ID`, return session token.
...
## 5. User Flow
1. `/plugin marketplace add cognifer-labs/cogwait` installs statusline + hooks.
2. Developer sets `COGWAIT_PAYOUT_ID` and accepts the workspace-trust prompt (required for statusline/hooks to run).
...
- **"Cogwait" backend:** must exist as a real network with viewable-ad inventory, or Cognifer builds it. Not assumed.
